...
#define LED_PIN     15
#define NUM_LEDS    55  
int BRIGHTNESS =    180; //value can be changed with rotary encoder
#define LED_TYPE    NEOPIXEL
//#define COLOR_ORDER GRB
CRGB leds[NUM_LEDS];

#define UPDATES_PER_SECOND 180

#define ROTARYSTEPS 3 // multiplier for how fast it adjusts the value
#define ROTARYMIN 0
#define ROTARYMAX 255
RotaryEncoder encoder(14, 32);
...
void loop()
{
  ...
  if (pos != newPos) {
    ...
    BRIGHTNESS = pos;
    EVERY_N_MILLISECONDS(200) { ChangePalettePeriodically(); };

    // send the message to the LEDs
...
//    if( secondHand ==  0)  { targetPalette = RainbowColors_p;   
    if( secondHand ==  0)  { targetPalette = October_Sky_gp;   
                             Serial.println("sky 45 for Owen"); 
                              }
if( secondHand == 16)  { targetPalette = Quick_Vent_gp;    
 ...
 /*   if( secondHand == 10)  { targetPalette = CRGBPalette16( g,p,b,bl, p,p,bl,p, p,g,b,b, bl,p,bl,b);    
   ...
   // }
       if( secondHand == 20)  { targetPalette = Warm_summer_day_gp;    
    Serial.println("sky 33 for Savannah"); 
    }
    if( secondHand == 36)  { targetPalette = Quick_Vent_gp;    
    Serial.println("wild orange"); 
    }
    if( secondHand == 40)  { targetPalette = Warm_summer_day_gp;    
    Serial.println("molten lava"); 
    }
    if( secondHand == 56)  { targetPalette = Quick_Vent_gp;    
